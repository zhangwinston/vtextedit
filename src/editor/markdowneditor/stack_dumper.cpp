...
StackDumper::StackDumper() {
    ...
    stackframe_.AddrPC.Offset = context_.Rip;  // Fill in register addresses (EIP, ESP, EBP).
    stackframe_.AddrPC.Mode = AddrModeFlat;
    stackframe_.AddrStack.Offset = context_.Rsp;
    stackframe_.AddrStack.Mode = AddrModeFlat;
    stackframe_.AddrFrame.Offset = context_.Rbp;
...
std::string StackDumper::DumpStack() {
    ...
    while (StackWalk64(IMAGE_FILE_MACHINE_AMD64, process_, thread_, &stackframe_,
