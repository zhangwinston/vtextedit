...
qreal TextEditUtils::getTextWidth(const QTextCharFormat chf, QString text)
{
    QFont f = chf.font();
    QFontMetrics fm(f);
...
void TextEditUtils::rangeVisibleChange(RangeInfo* range,LineInfo lineInfo,int cursor_pos)
{
    QRegularExpression re;
    QRegularExpressionMatch match;
    QTextBlock *pblock=lineInfo.m_pblock;
    QTextLine *pline=lineInfo.m_pline;

    QString text=lineInfo.m_pblock->text().mid(range->m_start,range->m_len);
    range->m_total_width=getTextWidth(range->m_chf,text);

    //default range content is visible
    range->visible_changed=false;
    range->m_visible_width=range->m_total_width;

    //heading line hide "# "
    if(cursor_pos < pblock->position() || (cursor_pos >(pblock->position()+ pline->textLength()))){
        re.setPattern("^#+ +");
        match= re.match(text);
        if(match.hasMatch()){
            range->visible_changed=true;
    ...
    //code inline hide "`"
    if(cursor_pos < (pblock->position()+pline->textStart()) ||
            (cursor_pos >(pblock->position()+pline->textStart()+ pline->textLength()))){
        re.setPattern("[^\\\\](`)");
        match = re.match(text);
        if(match.hasMatch()) {
            range->visible_changed=true;
...
void TextEditUtils::getRanges(LineInfo lineInfo,QVector<RangeInfo> &p_ranges,int pos)
{
//    qWarning()<<" lineInfo line nubmer: "<<lineInfo.m_line_num <<"total_line_flag "<< total_line_flag;
//    qWarning()<<" lineInfo line_text: "<<lineInfo.m_pblock->text().mid(lineInfo.m_pline->textStart(),lineInfo.m_pline->textLength());
    QVector<RangeInfo> ranges;

    if(lineInfo.m_all_line && lineInfo.m_pline->textLength()<1) return;
    if(lineInfo.m_all_line==false && lineInfo.m_part_len < 1)return;

    QVector<QTextLayout::FormatRange> fmt = lineInfo.m_pblock->layout()->formats();

    //qWarning()<<" line_start: "<< start <<" line_end:"<< end;

    //每行倒序绘制range，从尾部开始绘制
    int start=0;
    int end=0;
    if(lineInfo.m_all_line){
        start=lineInfo.m_pline->textStart();
        end=lineInfo.m_pline->textStart()+lineInfo.m_pline->textLength();
    }else{
        start=lineInfo.m_part_begin;
        end=lineInfo.m_part_begin+lineInfo.m_part_len;
    }
    ...
    do{
        ...
        //qWarning()<<" range_start:"<< range_start <<" range_end"<< range_end;

        for(; idx >= 0; idx--){
            auto fmt_start=fmt.at(idx).start;
            auto fmt_end=fmt_start+fmt.at(idx).length;

            //qWarning()<<" fmt_start: "<< fmt.at(idx).start <<" fmt_end:"<< fmt_end;
            //qWarning()<<" range_start: "<< range_start <<" range_end: "<< range_end;
            ...
            if(range_end < fmt_start){ //fmt behind line
                ...
                rangeVisibleChange(&range,lineInfo,pos);
                ranges.append(range);
                break;
        ...
        if(range_get_fmt==false){ //no fmt fit to this line item
            RangeInfo range;
            range.m_chf=lineInfo.m_pblock->charFormat();
            range.m_start=range_start;
            range.m_len=range_end-range_start;
    ...
    for(int i=ranges.count()-1; i>=0;i--){
        p_ranges.append(ranges.at(i));
//        qWarning()<<"range i="<<i;
//        qWarning()<<"range visible_text: "<< ranges.at(i).m_visible_text<<" visible_width: "<< ranges.at(i).m_visible_width;
//        qWarning()<<"range_total_width: "<< ranges.at(i).m_total_width;
//        qWarning()<<"range_chf: "<< ranges.at(i).m_chf.foreground().color().name();
    }
    return;
}

RangeInfo TextEditUtils::getRangesWidth(QVector<RangeInfo> &p_ranges)
{
    RangeInfo result;
    for(int i=0;i<p_ranges.count();i++){
        result.m_visible_width+=p_ranges.at(i).m_visible_width;
        result.m_total_width+=p_ranges.at(i).m_total_width;
    }
    return result;
}
