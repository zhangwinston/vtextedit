  ...
  for (auto it = fontGroups.begin(); it != fontGroups.end(); ++it) {
    ...
    for (const auto &pair : ranges) {
      ...
      if (range->m_processId != BLANKED_ID) {
        // Baseline-align to the QTextLine, matching QTextLayout::draw().
        // AlignVCenter inside the line box misplaces ranges that use a
        // different font (e.g. VERBATIM / inline code monospace) because
        // their ascent/height differ from the body font.
        const QPointF baseline(position.x(), linePosition.y() + li->m_tl.ascent());
...
void BlockLinesData::drawSelectionsOptimized(QPainter *painter, const QPointF &position,
  ...
  for (int i = 0; i < selections.size(); ++i) {
    ...
    for (int line = firstLine; line < lastLine; ++line) {
      ...
      // 使用 position().y() 而不是 naturalTextRect().y()，确保空白行和有文本行的垂直位置一致
      QRectF lineRect(tl.position().x(), tl.position().y(), li.m_width, tl.height());
      lineRect.translate(position);

      // 优化选择区域计算
      bool isFullWidth = selection.format.boolProperty(QTextFormat::FullWidthSelection);
      if (isFullWidth) {
        ...
        // 对于全宽选择，将行矩形扩展到整行宽度
        // 使用一个很大的值来覆盖整行，确保背景色延伸到行尾
        qreal fullWidth = INT_MAX / 256;
        if (clip.isValid()) {
          // 如果有裁剪区域，至少扩展到裁剪区域的右边
          fullWidth = qMax(fullWidth, clip.right() - lineRect.left());
        }
        lineRect.setWidth(fullWidth);
      }
