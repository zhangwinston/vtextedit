    ...
    if (block.userState() < 0) { // not code block
      ...
      while (matchi.hasNext()) {
        ...
        if ((block_len - pos > custom_str_len) &&
            (strView.mid(pos, custom_str_len).startsWith(QLatin1String("vx_images")) ||
             strView.mid(pos, custom_str_len).startsWith(QLatin1String("http")))) {
...
void BlockLinesData::addSelectedRegionsToPath(LineInfo *li, const QPointF &pos,
  ...
  // 对于全宽选择，使用 boundingRect 的完整宽度，从行的开始位置绘制
  bool isFullWidth = selection->format.boolProperty(QTextFormat::FullWidthSelection);
  if (isFullWidth) {
    // 全宽选择：从 boundingRect 的左边开始，使用完整宽度
    const QRectF rect = QRectF(boundingRect.x(), position.y(), boundingRect.width(), lineHeight);
    region->addRect(rect.toAlignedRect());
  } else if (selection_width > 0) {
    // 普通选择：使用计算的选择宽度和偏移
